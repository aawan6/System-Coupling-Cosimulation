...
def interpolv(x_data, y_data, x):
    ...
    return y_data[index] + (x - x_data[index]) * (y_data[index + 1] - y_data[index]) / (
        x_data[index + 1] - x_data[index]
    )
...
def interpolm(x_data, y_data, z_data, x, y):
    ...
    val_inter1 = z_data[index_x, index_y] + (x - x_data[index_x]) * (
        z_data[index_x + 1, index_y] - z_data[index_x, index_y]
    ) / (x_data[index_x + 1] - x_data[index_x])
    val_inter2 = z_data[index_x, index_y + 1] + (x - x_data[index_x]) * (
        z_data[index_x + 1, index_y + 1] - z_data[index_x, index_y + 1]
    ) / (x_data[index_x + 1] - x_data[index_x])

    return val_inter1 + (y - y_data[index_y]) * (val_inter2 - val_inter1) / (
        y_data[index_y + 1] - y_data[index_y]
    )
