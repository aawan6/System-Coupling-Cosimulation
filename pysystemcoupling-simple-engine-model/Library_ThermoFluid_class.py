...
#   - Source of Effort (Se)
#   - Source of Flow (Sf)
from Fluid_properties_class import FluidBGM
from EffortFlowPort_class import EffortTF, FlowTF, EffortM
from Solver import Integrator
from Data_treatment import interpolm, interpolv
...
class EffortSource(FluidBGM):
    ...
    def __init__(self, P, T, nC=10.8, nH=18.7, FAR=0):
        ...
        self.T = T
        self.E = EffortTF(
            self.P, self.T, self.h, self.u, self.cp, self.gamma, self.R, FAR
        )
...
class PressureLosses_R:
    ...
    def Solve(self):
        if (self.E1.P - self.E2.P) >= 0:
            self.Qm = (
                (self.E1.P / self.E1.T / self.E1.R)
                * self.K_PressureLosses
                * (self.E1.P - self.E2.P) ** 0.5
            )
            self.Qmh = self.Qm * self.E1.h
        else:
            self.Qm = (
                -(self.E2.P / self.E2.T / self.E2.R)
                * self.K_PressureLosses
                * (self.E2.P - self.E1.P) ** 0.5
            )
...
class Volume_C:
    ...
    def Solve(self, dt, method="Euler"):
        ...
        self.deltaT = (
            (delta_Qh - self.Fluid.u * self.delta_Qm)
            / self.m
            / (self.Fluid.cp - self.Fluid.R)
        )
...
class Compressor_Tf:
    ...
    def Solve(self):
        ...
        Qm_cor = max(
            1e-10, interpolm(self.y_margin, self.x_NC, self.z_flow_cor, surge, NC_cor)
        )
        self.Qm = Qm_cor * (self.E1.P / self.Prefc) / (self.E1.T / self.Trefc) ** 0.5

        # Outlet temperature calculation
        T2_is = (
            abs(self.E2.P / self.E1.P) ** ((self.E1.gamma - 1) / self.E1.gamma)
            * self.E1.T
        )
        self.Fluid = FluidBGM(self.nC, self.nH, self.FAR, T2_is)
        min_eta = 0.4
        etaC_is = max(
            min_eta, interpolm(self.y_margin, self.x_NC, self.z_eta_Comp, surge, NC_cor)
        )
...
class Turbine_TF:
    ...
    def Solve(self):
        ...
        Qm_cor = max(
            0, interpolm(self.x_PR, self.y_VNTposition, self.z_flowT_cor, PRT, self.VNT)
        )
        ...
        etaT_is = max(
            min_eta,
            interpolm(self.x_PR, self.y_VNTposition, self.z_eta_Turb, PRT, self.VNT),
        )
...
class Engine_Tf:
    ...
    def Solve(self):
        ...
        Eta_ind = interpolm(
            self.y_etavol_overall,
            self.x_NE,
            self.z_etaind,
            Eta_vol * self.E1.P * self.Trefm / self.Prefm / self.E1.T,
            self.Fm.N,
        )
        Eta_dT = interpolm(
            self.y_etavol_overall,
